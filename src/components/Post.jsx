...
import { ImageSlider, ProfileImage, TimeAgo, PostOptionsPopover } from "components";
...
export function Post({
  ...
  const { firstname, lastname, username, bookmarks } =
    postUser;
  ...
  return (
    <article className=" text-dark-2 flex w-full flex-col gap-2 rounded-lg bg-light-100 p-4 shadow-md dark:bg-dark-100 dark:text-gray-100">
        ...
        <div className="flex gap-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <small className="m-0 -mt-2 text-xs text-gray-400">
              <div className="tooltip tooltip-right" data-tip={createdAt}>
      ...
      {!isCommentPost ? (
        <footer onClick={(e) => e.stopPropagation()} className="flex justify-between">
          <button
            title={isPostLiked ? "Unlike" : "Like"}
            onClick={likeHandler}
            className={`${
              isPostLiked && "bg-primary/50 text-dark-100 "
            } align flex items-center gap-1 rounded-lg p-1 text-sm font-light text-primary transition-all hover:bg-primary/25`}
          >
            <FaThumbsUp />
